    ...
    while(1) {
        ...
        for (int i = 0; i < proc_count; i++) {
            ...
            if (entry->vflag == PAGE_VALID) {
                ...
                printf("[PID %02d IDX:%03d] %03d Page access: PF -> Allocated Frame %03d(Page fault인 경우)\n",
                       p->pid, p->ref_count - 1, page, f);
            }
        }
...
void print_page_tables(process *procs, int proc_count) {
    ...
    int total_faults = 0, total_refs = 0;

    for (int i = 0; i < proc_count; i++) {
        process *p = &procs[i];

        printf("** Process %03d: Allocated Frames=%03d PageFaults/References=%03d/%03d\n",
               p->pid, PAGETABLE_FRAMES + p->page_faults, p->page_faults, p->ref_count);
